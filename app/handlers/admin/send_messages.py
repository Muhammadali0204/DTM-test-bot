...
from app.utils.states import SendMessageStates
...
async def send_message_users(msg : Message, state : FSMContext):
    ...
    await msg.answer(
        ...
        reply_markup=admin_reply_keys.bekor_keyboard
    )
    clean_data(msg.from_user.id)
    await state.set_state(SendMessageStates.get_sending_message)

@router.message(SendMessageStates.get_sending_message, F.text == "❌ Bekor qilish")
...
@router.message(SendMessageStates.get_sending_message)
...
async def cancel_message(call : CallbackQuery):
    ...
    await call.message.answer('<b>Admin panel :</b>', reply_markup=admin_reply_keys.menu)
...
async def add_inline_button(call : CallbackQuery, state : FSMContext):
    ...
    if temp_data.get(f'{call.from_user.id}:inlines', None) is not None:
        if len(temp_data[f'{call.from_user.id}:inlines']) < 10:
            ...
            await state.set_state(SendMessageStates.get_btn_name_sending_msg)
        else:
            await call.answer(
...
@router.message(SendMessageStates.get_btn_url_sending_msg, F.text == "❌ Bekor qilish")
@router.message(SendMessageStates.get_btn_name_sending_msg, F.text == "❌ Bekor qilish")
...
@router.message(SendMessageStates.get_btn_name_sending_msg, F.content_type == ContentType.TEXT)
async def get_name(msg : Message, state : FSMContext):
    if len(msg.text) <= 40:
        ...
        await state.set_state(SendMessageStates.get_btn_url_sending_msg)
    else:
        await msg.answer(
...
@router.message(SendMessageStates.get_btn_url_sending_msg, F.content_type == ContentType.TEXT)
